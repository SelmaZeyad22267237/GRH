...
class EmploiyerController {
  static Future<List<Employee>> fetchEmploiyer() async {
    ...
    final response = await http.get( 
      Uri.parse('http://192.168.125.112:8000/rest/detail/'), 
    );
print(response.statusCode);
 ...
 var request = http.MultipartRequest( 
        'POST', Uri.parse('http://192.168.125.112:8000/rest/detail/'));
...
//Get.snackbar('title', 'add secsse',colorText: Colors.green,);
 ...
 var request = http.MultipartRequest(
        'PUT', Uri.parse('http://192.168.125.112:8000/rest/GPD_emp/${newEmployee.pk}'));
    request.files.add(http.MultipartFile.fromBytes(
        'photo', image!.readAsBytesSync(),
        filename: 'YourFilename.png'));
         request.fields["pk"] = "1";
...
//Get.snackbar('title', 'add secsse',colorText: Colors.green,);
 ...
 static Future<bool> delteEmploiyer(int pk) async {
  ...
  Uri.parse('http://192.168.125.112:8000/rest/GPD_emp/$pk'),
